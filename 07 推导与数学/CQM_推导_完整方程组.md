...
谱结构：能级 $E_n = 1/4 + \gamma_n^2$，耦级 $\mathfrak{c}_n$ 直接从 $\mathbb{H}^2$ 的几何涌出（详见 §七）。
...
> **此表达式是GL(5)整体的反映**。源于SU(5)重组实现⇒$A_4$嘉当矩阵→4紧致本征群$\{SU(2)_k\}_{k=1}^{4}$（全紧致）的整体结构，$\alpha$来自$U(1)$电磁群耦合常数。
