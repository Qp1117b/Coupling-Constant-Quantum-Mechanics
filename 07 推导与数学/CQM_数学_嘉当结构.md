...
**关联文档**：`CQM_推导_完整方程组.md`（方程总表）、`归档 CNT/06 论文/01-闭合核理论（CNT）` §4–§5（嘉当方程与 SU(5) 几何定理）
...
$$G_N = \frac{I\cdot\lambda_c\cdot \mathscr{C}^2\cdot \mathfrak{c}_1}{m_p^2}\cdot\exp\left(-\frac{2}{\mathscr{C}}\right)\cdot(1+\kappa \mathscr{C}),\quad \kappa = \frac{31+\mathscr{C}}{30}$$
...
4-单纯形（5 顶点、10 边、10 三角面、5 四面体胞腔）的边-面关联矩阵 $E \in \{0,1\}^{10\times 10}$：
...
耦合强度 $\gamma_p = 1/p$——圈图展开的递归折损因子：强力 ($p=2$) 折损 50% 最强，电磁力 ($p=5$) 折损 20% 最弱。
...
哈密顿量 Ĥ_∞（谱结构）
 → 离散化：嘉当方程（A₄ Cayley）
 → 嘉当矩阵 A₄ 本征结构 → 4-单纯形曲率算符 → S₅ 表示
 → SU(5) ⇒ U(1)×SU(2)×SU(3) 分解
 → p 进扇区 p=2,3,5 编码 → 规范场内容
 → 能动张量 T_μν → GR 涌现
```

该链与因果集路径（`归档 CNT/06 论文/01-闭合核理论（CNT）` §3–§6）完全一致，且仅因果集路径能为每一步提供"从何而来"的本体论回答。经典路径（`CQM_推导_经典路径.md`）提供数学骨架，但无法独立确立。
