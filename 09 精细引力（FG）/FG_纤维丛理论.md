...
- **公理 2（约束子群性质）**：$H \subset G$ 是闭子群。$H$ 闭 $\Rightarrow$ $G/\!\!/H$ 是紧致齐性空间（可用于真空流形，如 $SU(2)/\!\!/U(1)\cong S^2$，$H$ 不正规）；$H$ 闭且正规 $\Rightarrow$ $G/\!\!/H$ 是紧致李群（可用于新结构群，如 $U(1)/\!\!/Z_2\cong U(1)$，$H$ 正规）。
...
$//H$ 编码荷的量子化（拓扑折叠，如 $2e, 4e, ne$），$\rho$ 编码物质态的空间 $V$（维度 = 态的个数，变换规则 = 物质类型），$R$ 编码约束涌现的结构群（紧致李群或齐性空间）。

**关键定理**：

1. **约束商性质**：若 $G$ 紧致且 $H$ 闭，则 $G/\!\!/H$ 紧致。若 $H \triangleleft G$，则 $G/\!\!/H$ 是紧致李群。（紧致性判据的五种情形见 §1.5.3。）
...
| $U(1)/\!\!/Z_n \xrightarrow{\rho_q} U(1)$ | $U(1)$ | 带 $q$ 荷的物质 | 磁通量子化 |
| $SU(2)/\!\!/U(1) \xrightarrow{\rho_{\text{向量}}} S^2$ | $S^2$ | $W^\pm$ | 电弱破缺 |
| $SU(3)/\!\!/U(2) \xrightarrow{\rho_{\text{三重态}}} \mathbb{CP}^2$ | $\mathbb{CP}^2$ | 夸克（3态） | 色约束 |
| $SO(3)/\!\!/SO(2) \xrightarrow{\rho_{\text{旋量}}} S^2$ | $S^2$ | 磁振子（2态） | 铁磁体 |
| $GL(5)/\!\!/SU(5) \xrightarrow{\rho_{\text{自守}}} SU(5)\text{ 表示}$ | $SU(5)$ 表示 | 同步谱 | QG 紧化 |
...
最后一行 $GL(5)/\!\!/SU(5) \to SU(5)$ 是 QG 层紧化：$GL(5)$（非紧约化群，自由相）经 $SU(5)$ 约束紧化为物理紧群 $SU(5)$，自守表示 $\rho_{\text{自守}}$ 给出同步谱（§1.5.3 类型 I 正规商）。$GL(5)$ 的 $n=5$ 由涨落/破缺事件落在的群指定（见 `FG_核心理论.md` §3.0），不由底空间指定。
