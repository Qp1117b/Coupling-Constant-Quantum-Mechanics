...
三种子类型（统一于 $/\!\!/ $，均满足公理 1+2：$G$、$H$、$R$ 紧致李群，$H \triangleleft G$）：

| 类型 | 约束条件 | $G /\!\!/ H = R$ | 物理对应 |
|------|---------|-------------------|---------|
| **I 正规商** | $H \triangleleft G$ | 李群 $G/H$ | 磁通量子化（$U(1)/\!\!/Z_n$） |
| **III 核盲视** | $H = \ker(\rho)$（自动正规） | $\rho(G) \subset GL(V)$ | 整数自旋表示（$SU(2)/\!\!/Z_2 \cong SO(3)$） |
| **IV 对角锁定** | $H \subset Z(G_1\times G_2)$（中心，自动正规） | 直积中心商 | 电荷量子化（$U(N) = (U(1)\times SU(N))/\!\!/Z_N$） |

类型 II（齐性破缺，$H$ 不正规）和类型 V（结构约化，$G$ 非紧致）不满足公理 1+2，不是约束 $//$：类型 II 是真空选择（连续破缺在 $V$ 中 VEV 处理），类型 V 是紧化投影（非紧群取极大紧子群，如 QG 层 $GL(5)\to SU(5)$，不是商群操作）。

**三种子类型均满足 $H \triangleleft G$**：

1. **类型 I（正规商）**：$H \triangleleft G$，$H$ 闭。此时 $G/\!\!/H = G/H$ 是商群，紧致性由 $G$ 继承。例：$U(1)/Z_n \cong U(1)$、$SU(2)/Z_2 \cong SO(3)$、$SU(N)/Z_N \cong PSU(N)$。

2. **类型 III（核盲视）**：$H = \ker(\rho)$，$\rho: G \to GL(V)$ 连续。$\ker(\rho)$ 自动正规（对任意 $h \in \ker(\rho)$ 与 $g \in G$，$\rho(ghg^{-1}) = \rho(g)\rho(h)\rho(g)^{-1} = I$，故 $ghg^{-1} \in \ker(\rho)$）。$G/\!\!/H \cong \rho(G) \subset GL(V)$，紧致群的连续同态像仍是紧致李群。例：$SU(2) \xrightarrow{\text{自旋-1}} SO(3) \subset GL(3,\mathbb{R})$（核 $Z_2$）；$U(1) \xrightarrow{\rho_n} U(1) \subset GL(1,\mathbb{C})$（核 $Z_n$）。类型 III 是类型 I 的特例（$\ker(\rho) \triangleleft G$）。

3. **类型 IV（对角锁定）**：$H \subset Z(G_1 \times G_2)$，$H$ 闭。中心子群自动正规。$(G_1 \times G_2)/H$ 是中心商，自然紧致。例：$U(N) = (U(1) \times SU(N))/Z_N$、$\mathrm{Spin}^c(n) = (\mathrm{Spin}(n) \times U(1))/Z_2$。类型 IV 是类型 I 的特例（中心子群正规）。

综上：约束 $//$ 要求 $G$、$H$、$R$ 均为紧致李群且 $H \triangleleft G$，三种类型（正规商、核盲视、对角锁定）中 $H$ 均正规。不满足者不是约束 $//$：$H$ 不正规是真空选择（§1.5.8 公理 2），$G$ 非紧致是紧化投影（QG 层单独处理）。
...
| **纤维 $F$** | 向量空间 $V$（$\mathbb{C}, \mathbb{C}^n, \mathfrak{g}$） | 紧致李群 $R = G /\!\!/ H$（$H \triangleleft G$） |
...
### 1.5.8 动力学纤维丛：$G/\!\!/H \xrightarrow{\rho} R$（运动学与动力学的统一）

约束框架的完整符号是**约束商与表示的复合**，同一符号同时表达两个框架：

$$\boxed{\underbrace{G/\!\!/H}_{\text{动力学（约束）}} \;\underbrace{\xrightarrow{\rho}}_{\text{运动学（表示）}} \; R}$$

**读作**：$G$ 经约束 $H$ 涌现出结构群 $R$（动力学），随后 $R$ 在表示 $\rho$ 下生成物质（运动学）。
...
| 步骤 | 数学实体 | 编码什么 | 框架 |
|:---|:---|:---|:---|
| ① 约束 | $G/\!\!/H$ | 荷的量子化（拓扑折叠） | 动力学 |
| ② 结果 | $R = G/\!\!/H$ | 约束涌现的结构群 | 动力学 |
| ③ 表示 | $\rho: R \to GL(V)$ | 物质态的数量与变换规则 | 运动学 |
| ④ 伴丛 | $R\times_\rho V$ | 物质场的承载空间 | 运动学 |
| ⑤ 物质 | $\Gamma(R\times_\rho V)$ | 携带 $R$ 之荷的场 | 运动学 |
...
|:---|:---|:---|
| $G \to H$ | 破缺过程 | 剩下什么对称性 |
| $G/H$ | 齐性空间（先验舞台） | 破缺后有哪些真空方向 |
| $G/\!\!/H$ | 约束商 | 空间从内部等价关系涌现 |
| $G/\!\!/H \xrightarrow{\rho} R$ | 动力学纤维丛 | **约束定群，表示定物质** |

**核心原则**：

$$\boxed{/\!\!/ \text{ 定 } R \text{（约束产物/荷），} \rho \text{ 定物质类型（态的数量与规则），二者不重复}}$$

$//H$ 编码荷的量子化（拓扑折叠，如 $2e, 4e, ne$），$\rho$ 编码物质态的空间 $V$（维度 = 态的个数，变换规则 = 物质类型），$R$ 编码约束涌现的紧致李群。

**关键定理**：

1. **约束商性质**：若 $G$ 紧致连通且 $H \triangleleft G$ 闭正规，则 $G$、$H$、$R = G/\!\!/H$ 三者均为紧致李群（公理 1+2 直接推论，$H$ 紧致性由紧致群闭子群自动给出）。连续破缺 $R\to R'$ 不在约束层发生，在表示层 $V$ 中通过 VEV 实现。（紧致性判据的三种类型见 §1.5.3。）
2. **线性化退化**：若 $\rho$ 忠实且 $\ker\rho = H$，则约束伴丛与容器伴丛同构：$P \times_G (G/H) \cong P \times_\rho V$（§1.5.7 退化情形）。表示论是约束框架在真空选定后的线性化极限。
3. **自由度丰富性**：总自由度 $\sim \dim V \cdot \dim M$，热力学极限下截面空间无穷维。作用量谱包含环面量子数格、Casimir 格、离散商拓扑刻度，可数无限丰富。
4. **阿贝尔约束的表示限制**：若 $R = U(1)/\!\!/Z_n \cong U(1)$ 是阿贝尔群，则其不可约表示的 $V = \mathbb{C}$（一维）。约束 $Z_n$ 只改变荷的量子化单位（$e \to ne$），不改变 $V$ 的维度。
5. **复合约束的逐因子分解**：若 $G = G_1\times\cdots\times G_k$，$H = H_1\times\cdots\times H_k$（直积子群），则 $G/\!\!/H \cong (G_1/\!\!/H_1) \times \cdots \times (G_k/\!\!/H_k)$。

**$GL(V)$ 与态的数量**：$GL(V) \cong GL(n, \mathbb{K})$，当 $\dim V = n$。$V$ 是物质内部空间（态的容器），$\dim V = n$ 是态的个数。$GL(V)$ 是内禀写法（不依赖基底），$GL(n)$ 是坐标写法（依赖基底选取）。$\rho$ 一旦选定，$V$ 自动锁定——$V$ 脱离 $\rho$ 不构成物质。**物质的本质 = 态的数量 + 态在 $R$ 下的变换规则**。

| 物质 | $V$ | $\dim V$ | $GL(V)$ |
|:---|:---|:---:|:---|
| 标量（库珀对、希格斯） | $\mathbb{C}$ | 1 | $GL(1)$ |
| 旋量（电子） | $\mathbb{C}^2$ | 2 | $GL(2)$ |
| 夸克（色三重态） | $\mathbb{C}^3$ | 3 | $GL(3)$ |
| 胶子（八重态） | $\mathbb{C}^8$ | 8 | $GL(8)$ |

**与 CQM 朗兰兹分层的呼应**：

| 来源 | 紧致群 $R$ | 基本表示 $V$ | $\dim V$ | $GL(n)$ | 在文档中的角色 |
|:---|:---|:---|:---:|:---|:---|
| $SU(5)$ 整体 | $SU(5)$ | $\mathbb{C}^5$ | 5 | $GL(5)$ | 基态同步（单层自守表示） |
| $SO(5)$ $K$-type | $SO(5)\subset SU(5)$ | 旋量表示 $\mathbb{C}^4$ | 4 | $GL(4)$ | $K$-type（角动量层） |
| 色因子破缺 | $SU(3)$ | $\mathbb{C}^3$ | 3 | $GL(3)$ | 色因子层 |
| 模对称破缺 | $SU(2)$ | $\mathbb{C}^2$ | 2 | $GL(2)$ | 模对称层 |
| 电磁因子破缺 | $U(1)$ | $\mathbb{C}$ | 1 | $GL(1)$ | 中心特征（$\zeta(s)$） |

$$\boxed{GL(n) \text{ 的 } n \;\Longleftrightarrow\; \text{基本表示的态数} = \dim V}$$

这与 `FG_核心理论.md` §3.0 的 GL(n) 来源一致：约束产物 $R$ 通过基本表示 $\rho_{\text{fund}}: R \to GL(V_{\text{fund}})$ 严格确定 $n = \dim V_{\text{fund}}$（表示的像决定 $n$，非群标签匹配）。

**统一图景**：

$$\underbrace{SU(5) \xrightarrow{\rho_5} GL(5)}_{\text{整体自守}} \;\supset\; \underbrace{SO(5) \xrightarrow{\rho_4} GL(4)}_{K\text{-type}} \;\xleftarrow{\text{破缺}}\; \underbrace{U(1)\times SU(2)\times SU(3) \xrightarrow{\rho_1\oplus\rho_2\oplus\rho_3} GL(1)\times GL(2)\times GL(3)}_{\text{规范因子层}}$$
...
| | 容器框架（运动学） | 动力学纤维丛（CQM） |
|:---|:---|:---|
| 底空间 | 先验给定 $M$ | **约束涌现** $R = G/\!\!/H$ |
| 结构群 | 固定 $G$ | **约束生成** $R$ |
| 表示 $\rho$ | 输入参数 | **约束后指定** |
| 约束 | 外部微分方程 | **内部等价关系** |
| 物质 | 向量值截面 | **序参量截面 = 携 $R$ 之荷的场** |

**完整实例表**：

| 符号 | $R = G/\!\!/H$ | 物质 | 物理 |
|:---|:---|:---|:---|
| $U(1)/\!\!/Z_2 \xrightarrow{\rho_{\text{标量}}} U(1)$ | $U(1)$ | $\Delta$（库珀对，自旋0，1态） | 超导 |
| $U(1)/\!\!/Z_n \xrightarrow{\rho_q} U(1)$ | $U(1)$ | 带 $q$ 路的物质 | 磁通量子化 |
| $SU(2)/\!\!/Z_2 \xrightarrow{\rho_{\text{向量}}} SO(3)$ | $SO(3)$ | $W^\pm$（向量玻色子，3态） | 电弱（约束层） |
| $SU(3)/\!\!/Z_3 \xrightarrow{\rho_{\text{三重态}}} PSU(3)$ | $PSU(3)$ | 夸克（3态） | 色约束 |
| $SO(3)/\!\!/\{e\} \xrightarrow{\rho_{\text{向量}}} SO(3)$ | $SO(3)$ | 磁振子（3态） | 铁磁体（约束层） |
| $GL(5)/\!\!/SU(5) \xrightarrow{\rho_{\text{自守}}} SU(5)$ | $SU(5)$ | 同步谱 | QG 紧化 |

注：电弱破缺 $SU(2)\times U(1)\to U(1)_{\text{em}}$ 与铁磁体 $SO(3)\to SO(2)$ 的连续破缺不在约束层发生，而在表示层 $V$ 中通过真空期望值实现（见下文"真空破缺"）。上表约束层的 $SU(2)/\!\!/Z_2\cong SO(3)$ 和 $SO(3)/\!\!/\{e\}\cong SO(3)$ 只生成结构群，连续破缺随后在 $V$ 内部发生。

**实例详解**：

**库珀对**（$U(1)/\!\!/Z_2 \xrightarrow{\rho_{\text{标量}}} U(1) \to \Delta$）：① 约束 $U(1)/\!\!/Z_2$：相位 $\theta\sim\theta+\pi$，$2e$ 量子化涌现；② 结构群 $R=U(1)$；③ 表示 $\rho_{\text{标量}}: R \to GL(\mathbb{C})$：自旋 0，态数 1；④ 伴丛 $U(1)\times_\rho\mathbb{C}$；⑤ 物质 $\Delta$：库珀对场，带 $2e$，自旋 0。**关键分工**：$//Z_2$ 编码 $2e$（约束），$\rho_{\text{标量}}$ 编码自旋 0 和态数 1（表示），二者不重复。

**$Z_n$ 一般情形**（$U(1)/\!\!/Z_n \xrightarrow{\rho_{nm}} U(1)$）：

| 约束 | 相位周期 | 最小非零电荷 | 磁通量子 | $V$ |
|:---:|:---:|:---:|:---:|:---:|
| $Z_2$ | $\pi$ | $2e$ | $h/2e$ | $\mathbb{C}$ |
| $Z_3$ | $2\pi/3$ | $3e$ | $h/3e$ | $\mathbb{C}$ |
| $Z_n$ | $2\pi/n$ | $ne$ | $h/ne$ | $\mathbb{C}$ |

$Z_n$ 只改变荷的刻度，不改变 $V$ 的维度（定理 4）。$V = \mathbb{C}$ 恒定。

**$U(1)\times SU(2)/\!\!/Z_2$ 对角锁定**（电弱典型）：$Z_2=\{(-1,-1)\}$ 对角嵌入中心，$R\cong U(1)\times SO(3)$（紧致李群，公理 1+2 直接推论）。表示筛选：$\rho_{(q,j)}(-1,-1)=(-1)^{q+2j}=1 \Rightarrow q+2j$ 偶数。

| $(q,j)$ | $q+2j$ | 允许 | 物理 | $V$ | 态数 |
|:---:|:---:|:---:|:---|:---|:---:|
| $(0,0)$ | 0 | 是 | 中性标量 | $\mathbb{C}$ | 1 |
| $(1,1/2)$ | 2 | 是 | 轭子二重态 | $\mathbb{C}^2$ | 2 |
| $(2,0)$ | 2 | 是 | 库珀对 | $\mathbb{C}$ | 1 |
| $(1,0)$ | 1 | 否 | **禁止** | — | — |
| $(0,1/2)$ | 1 | 否 | **禁止** | — | — |

**复合约束**：直积子群约束 $(G_1\times G_2)/\!\!/(H_1\times H_2) \cong (G_1/\!\!/H_1)\times(G_2/\!\!/H_2)$，$V = V_1\otimes V_2$，$\dim V = \dim V_1\cdot\dim V_2$（定理 5）。对角锁定（斜嵌入，非直积）$H\subset G_1\times G_2$ 但 $H\not\subset G_1$ 也 $H\not\subset G_2$，不能逐因子分解，产生跨因子量子化条件——这是最深刻的约束类型。

**标准模型对角 $Z_6$**（$(U(1)\times SU(2)\times SU(3))/\!\!/Z_6 \xrightarrow{\rho_{(Y,I,C)}} (U(1)\times SU(2)\times SU(3))/Z_6$）：$Z_6 = \{(e^{2\pi ik/6}, (-1)^k, e^{2\pi ik/3}I_3) \mid k=0,\ldots,5\}$ 对角嵌入。约束条件（电荷量子化的根源）：$Y + 2I + C_3 \equiv 0 \pmod{6}$。涌现的 $R$ 是标准模型规范群的真正全局形式，紧致李群（公理 1+2）。

| 物质 | $(Y,I,C)$ | $V$ | $GL(V)$ |
|:---|:---|:---|:---|
| 轭子 | $(1,1/2,\mathbf{1})$ | $\mathbb{C}\otimes\mathbb{C}^2$ | $GL(2)$ |
| 夸克 | $(1/3,1/2,\mathbf{3})$ | $\mathbb{C}\otimes\mathbb{C}^2\otimes\mathbb{C}^3$ | $GL(6)$ |
| 胶子 | $(0,0,\mathbf{8})$ | $\mathbb{C}^8$ | $GL(8)$ |

最后一行 $GL(5)/\!\!/SU(5) \to SU(5)$ 是 QG 层紧化投影（非约束 $//$，因 $GL(5)$ 非紧致不满足公理 1）：$GL(5)$（非紧约化群，自由相）经紧化投影到极大紧子群 $SU(5)$，自守表示 $\rho_{\text{自守}}$ 给出同步谱。$GL(5)$ 的 $n=5$ 由紧化产物 $SU(5)$ 的基本表示 $V=\mathbb{C}^5$ 的 $\dim V = 5$ 严格确定（见 `FG_核心理论.md` §3.0），不由底空间指定。

**真空破缺（连续破缺在表示层 $V$ 中发生）**：约束 $//$ 生成紧致李群 $R$，连续破缺 $R\to R'$ 不在约束层发生，而在表示空间 $V$ 中通过真空期望值（VEV）实现。

$$\boxed{\text{约束 } /\!\!/ \text{ 生成群 } R\text{；真空破缺在 } V \text{ 内部发生}}$$

例（铁磁体）：$SO(3)/\!\!/\{e\} \xrightarrow{\rho_3} SO(3)$，$\rho_3: SO(3) \to GL(\mathbb{R}^3)$。物质是 $SO(3)$ 的三维表示。真空期望值 $\langle\psi\rangle = (0,0,v)$ 在表示空间 $\mathbb{R}^3$ 内部选择方向，这才是 $SO(3) \to SO(2)$ 破缺的起因。约束层只生成 $R = SO(3)$，破缺由 $V$ 中的 VEV 驱动。

例（电弱破缺）：$(U(1)\times SU(2))/\!\!/Z_2 \xrightarrow{\rho_{\text{Higgs}}} U(1)\times SO(3)$。约束层生成 $R = U(1)\times SO(3)$（$Z_2$ 在中心，正规）。Higgs 表示 $\rho_{\text{Higgs}}: R \to GL(\mathbb{C}^2)$，VEV $\langle\phi\rangle = (0, v/\sqrt{2})$ 在 $V = \mathbb{C}^2$ 中选择方向，破缺 $U(1)\times SU(2) \to U(1)_{\text{em}}$。

| 层级 | 内容 | 严格性 |
|:---|:---|:---|
| 约束 $//$ | $G/\!\!/H \to R$，$G$、$H$、$R$ 均紧致李群，$H \triangleleft G$ | 严格 |
| 表示 $\rho$ | $\rho: R \to GL(V)$ | 严格 |
| 真空破缺 | 若需连续破缺，在 $V$ 中取 VEV | 标准物理 |
| 物质 | $\Gamma(R\times_\rho V)$ | 严格 |

**自由度与态的数量**：连续自由度 $\dim(G/\!\!/H) = \dim G - \dim H$。总自由度 $\sim \dim V \cdot |M_\ell| \to \infty$（热力学极限）。作用量谱 $= \{\text{环面量子数格 } \mathbb{Z}^r\} \cup \{\text{Casimir 格}\} \cup \{\text{离散商 } Z_n \text{ 的刻度}\}$，可数无限丰富。

**发生学顺序**：

$$\boxed{\text{母群 } G \;\to\; \text{约束 } /\!\!/H \;\to\; \text{紧致李群 } R \;\to\; \text{表示 } \rho \;\to\; \text{态空间 } V \;\to\; \text{物质场}}$$

约束（$//$）先于表示（$\rho$），背景（$R$）先于物质（截面）。

**为什么是统一**：传统运动学纤维丛只写 $G \xrightarrow{\rho} GL(V) \to \text{伴丛} \to \text{物质}$，缺失底空间和荷的来源。CQM 动力学纤维丛补全前半段：

$$\boxed{\underbrace{G/\!\!/H}_{\text{补上：底空间和荷的来源}} \;\underbrace{\xrightarrow{\rho} R}_{\text{保留：物质的承载}}}$$

运动学是动力学约束完成后的极限，动力学纤维丛 $G/\!\!/H \xrightarrow{\rho} R$ 是两者的统一表述。

**容器与物质可能性的同时涌现**：约束 $//H$ 不只折叠出容器 $R$，还**同时**限定了在该容器上什么样的物质（表示 $\rho$）是被允许的——容器与物质可能性同源：

$$\boxed{G/\!\!/H \;\xrightarrow{\text{同时涌现}}\; \underbrace{R}_{\text{容器}} + \underbrace{GL(V)}_{\text{物质可能性}}}$$

标准框架中容器（主丛）和物质（截面）是**独立的外部输入**，没有一个生成另一个；动力学框架中两者都来自约束 $//H$。例（库珀对）：$U(1)/\!\!/Z_2$ 同时涌现容器 $U(1)$（相位周期 $\pi$）与物质可能性（只有 $q$ 偶数的表示存活，即库珀对 $2e$ 而非电子 $e$）。**约束同时生成容器和物质可能性，物质随后实现为截面。**
