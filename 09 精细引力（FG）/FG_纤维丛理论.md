...
### 1.5.8 完整符号体系：$G/\!\!/H \xrightarrow{\rho} R$

约束框架的完整符号是**约束商与表示的复合**，而非单独的 $G/\!\!/H$：

$$\boxed{G/\!\!/H \xrightarrow{\rho} R}$$

**读作**：$G$ 经约束 $H$，生成紧致李群（或齐性空间）$R$；在 $R$ 的表示 $\rho$ 下，涌现出携带 $R$ 之荷的物质场。

**完整展开链**：

$$\boxed{\underbrace{G/\!\!/H}_{\text{约束}} \;\longrightarrow\; \underbrace{R}_{\text{紧致李群}} \;\xrightarrow{\rho}\; \underbrace{R\times_\rho V}_{\text{伴丛}} \;\longrightarrow\; \underbrace{\Gamma(R\times_\rho V)}_{\text{物质}}}$$

| 步骤 | 数学实体 | 编码什么 |
|:---|:---|:---|
| ① 约束 | $G/\!\!/H$ | **荷的量子化**（拓扑折叠，如 $2e$） |
| ② 结果 | $R = G/\!\!/H$ | 约束涌现的结构群 |
| ③ 表示 | $\rho: R \to GL(V)$ | **物质的自旋/场类型**（标量/旋量/向量） |
| ④ 伴丛 | $R\times_\rho V$ | 物质场的承载空间 |
| ⑤ 物质 | $\Gamma(R\times_\rho V)$ | 携带 $R$ 之荷的场 |
...
| $G/\!\!/H$ | 约束商 | **空间从内部等价关系涌现** |
| $G/\!\!/H \xrightarrow{\rho} R$ | 约束+表示复合 | **约束定群，表示定物质** |

**核心原则**：

$$\boxed{/\!\!/ \text{ 定 } R \text{（约束产物/荷），} \rho \text{ 定物质类型（自旋/场），二者不重复}}$$

$//H$ 编码荷的量子化（拓扑折叠），$\rho$ 编码物质场的类型（标量/旋量/向量），$R$ 编码约束涌现的结构群。约束（$//$）先于表示（$\rho$），背景（$R$）先于物质（截面）——这是约束框架区别于容器框架（$R$ 先验给定、$\rho$ 为输入参数）的本质。

**发生学顺序**：

$$\boxed{\text{母群 } G \;\to\; \text{约束 } /\!\!/H \;\to\; \text{紧致李群 } R \;\to\; \text{表示 } \rho \;\to\; \text{物质场}}$$

**完整实例表**：

| 符号 | $R = G/\!\!/H$ | 物质 | 物理 |
|:---|:---|:---|:---|
| $U(1)/\!\!/Z_2 \xrightarrow{\rho_{\text{标量}}} U(1)$ | $U(1)$ | $\Delta$（库珀对） | 超导 |
| $U(1)/\!\!/Z_n \xrightarrow{\rho_q} U(1)$ | $U(1)$ | 带 $q$ 荷的物质 | 磁通量子化 |
| $SU(2)/\!\!/U(1) \xrightarrow{\rho_{\text{向量}}} S^2$ | $S^2$ | $W^\pm$ | 电弱破缺 |
| $SU(3)/\!\!/U(2) \xrightarrow{\rho_{\text{三重态}}} \mathbb{CP}^2$ | $\mathbb{CP}^2$ | 夸克 | 色约束 |
| $SO(3)/\!\!/SO(2) \xrightarrow{\rho_{\text{旋量}}} S^2$ | $S^2$ | 磁振子 | 铁磁体 |
