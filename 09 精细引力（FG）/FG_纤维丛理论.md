...
$//H$ 编码荷的量子化（拓扑折叠，如 $2e$），$\rho$ 编码物质态的空间 $V$（维度 = 态的个数，变换规则 = 物质类型），$R$ 编码约束涌现的结构群。

**关键定理**：

1. **约束商性质**：若 $G$ 紧致且 $H$ 闭，则 $G/\!\!/H$ 紧致。若 $H \triangleleft G$，则 $G/\!\!/H$ 是紧致李群。（紧致性判据的五种情形见 §1.5.3。）
2. **线性化退化**：若 $\rho$ 忠实且 $\ker\rho = H$，则约束伴丛与容器伴丛同构：$P \times_G (G/H) \cong P \times_\rho V$（§1.5.7 退化情形）。
3. **自由度丰富性**：总自由度 $\sim \dim V \cdot \dim M$，热力学极限下截面空间无穷维。作用量谱包含环面量子数格、Casimir 格、离散商拓扑刻度，可数无限丰富。

**$GL(V)$ 与态的数量**：$GL(V) \cong GL(n, \mathbb{K})$，当 $\dim V = n$。$V$ 是物质内部空间（态的容器），$\dim V = n$ 是态的个数，$\rho$ 决定 $V$ 的维度与 $R$ 在 $V$ 上的作用方式。**物质的本质 = 态的数量 + 态在 $R$ 下的变换规则**。

| 物质 | $V$ | 态的个数 |
|:---|:---|:---:|
| 标量（库珀对、希格斯） | $\mathbb{C}$ | 1 |
| 旋量（电子） | $\mathbb{C}^2$ | 2 |
| 夸克（色三重态） | $\mathbb{C}^3$ | 3 |
| 胶子（八重态） | $\mathbb{C}^8$ | 8 |
...
**库珀对实例展开**：$U(1)/\!\!/Z_2 \xrightarrow{\rho_{\text{标量}}} U(1) \to \Delta$。① 约束 $U(1)/\!\!/Z_2$：相位 $\theta\sim\theta+\pi$，$2e$ 量子化涌现；② 结构群 $R=U(1)$；③ 表示 $\rho_{\text{标量}}: R \to GL(\mathbb{C})$：自旋 0，态数 1；④ 伴丛 $U(1)\times_\rho\mathbb{C}$；⑤ 物质 $\Delta$：库珀对场，带 $2e$，自旋 0。**关键分工**：$//Z_2$ 编码 $2e$（约束），$\rho_{\text{标量}}$ 编码自旋 0 和态数 1（表示），二者不重复。
