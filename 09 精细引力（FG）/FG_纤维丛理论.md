...
&\text{约束群 } H \triangleleft G &&\text{—— 闭正规子群（约束触发）} \\
...
| **II 齐性破缺** | $H \subset G$ 闭 | 齐性空间 $G/H$（**真空选择，非约束 $//$**） | 自发对称性破缺（$SO(3)\to SO(2)$，在 $V$ 中取 VEV） |
...
**类型 II（齐性破缺）是真空选择，非约束 $//$**：若 $H$ 不正规且不是任何表示的核，则 $G/H$ 只是紧致齐性流形（如 $S^2$、$\mathbb{CP}^2$），不是群。在 CQM 动力学纤维丛框架中，这不叫约束 $//$，叫真空选择——连续破缺在表示层 $V$ 中通过 VEV 处理（§1.5.8 公理 2）。约束 $//$ 要求 $H \triangleleft G$，产物 $R$ 必须是紧致李群。
...
综上：$G/\!\!/H$ 是紧致李群，仅当 $H$ 是闭正规子群、表示核、或中心对角子群——三种情形中 $H$ 均正规。若 $H$ 不正规，$G/H$ 只是紧致流形，属于真空选择而非约束 $//$（§1.5.8 公理 2）。
...
- **公理 2（约束子群正规性）**：$H \triangleleft G$ 是闭正规子群。约束产物 $R = G/\!\!/H$ 必须是紧致李群，因此 $H$ 必须正规——此时 $R$ 紧致性由 $G$ 继承，群结构由商群定理保证。若 $H$ 不正规，$G/\!\!/H$ 只是齐性空间而非群，不叫约束 $//$，叫真空选择——连续破缺在表示层 $V$ 中通过真空期望值处理（见下文"真空破缺"）。
...
1. **约束商性质**：若 $G$ 紧致连通且 $H \triangleleft G$ 闭正规，则 $R = G/\!\!/H$ 是紧致李群（公理 1+2 直接推论）。连续破缺 $R\to R'$ 不在约束层发生，在表示层 $V$ 中通过 VEV 实现。（紧致性判据的五种情形见 §1.5.3。）
...
| 层级 | 内容 | 严格性 |
|:---|:---|:---|
| 约束 $//$ | $G/\!\!/H \to R$，$H \triangleleft G$，$R$ 紧致李群 | 严格 |
