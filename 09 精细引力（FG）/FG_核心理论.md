...
> **定位注**：作用量算符（同步算符）在**功能角色**上对应模形式理论中 Hecke 算符的本征筛选——两者都是"从底空间函数空间中挑选本征形式"的算符——但**数学对象不同**：Hecke 算符 $T_p$ 是算术算符（双陪集定义，本征值 $a_p(f)$ 编码 L 函数系数），同步算符是谱/动力学算符（玻尔-索末菲量子化，本征值 = 量子化作用量 $J/2\pi$）。二者是功能对应，非数学等同；自守形式的唯一确定仍需权（K-类型）、Hecke 本征特征等额外数据（Hecke 本征时由强多重性一定理保证唯一，见 `01 核心理论/CQM_核心_朗兰兹分层共振与相变量子.md` §0.5）。
...
3. **黎曼猜想的物理入口**：黎曼猜想等价于配对不变量 $\mathfrak{c}_n=\rho_n(1-\rho_n)$ 为实，等价于物质同步稳定性
