...
FG的核心问题之一是：**给定底空间（Regge剖分）+条件（算符+对称性），求解同步方程的自守形式，本征群 $R$（重组产物）对应的耦合常数就是涨落耦合常数指定的。**
