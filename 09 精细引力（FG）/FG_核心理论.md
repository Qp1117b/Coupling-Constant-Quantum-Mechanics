...
> **GL(5)大统一框架**：$GL(5)$ 是真正的宇宙大统一群（约化李群，非紧致），有两个实现：$GL(5,\mathbb{C})$（含 $SU(5)$）和 $GL(5,\mathbb{R})$（含洛伦兹群 $SO(1,3)$）。**$GL(5)$ 本身是重组实现的，不是破缺的**——这说明：破缺的一定是非紧致李群，但实现重组实现的既可以是紧致李群（如 $SU(5)$）也可以是非紧李群（如 $GL(5)$ 本身）。$GL(5,\mathbb{R})$ 的时空对称性若纳入规范群则必须破缺（产生负动能，需破缺或冻结）。$GL(5)$ 本身是物质更前面的自组织的产物。规范群必须紧致，故物理上可行的大统一规范群候选是 $SU(5) \subset GL(5,\mathbb{C})$。
