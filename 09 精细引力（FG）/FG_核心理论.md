...
> **GL(5)大统一框架**：$GL(5)$ 是真正的宇宙大统一群（约化李群，非紧致），有两个实现：$GL(5,\mathbb{C})$（含 $SU(5)$）和 $GL(5,\mathbb{R})$（含洛伦兹群 $SO(1,3)$）。**$GL(5)$ 本身是重组实现的，不是破缺的**——就物理相变而言：破缺的必须是非规范紧致李群（或更一般地，非规范对称性），但非规范紧致李群不一定破缺；重组实现的既可以是规范紧致李群，也可以是非规范紧致李群；而规范紧致李群相变必然是重组，而非破缺。$GL(5,\mathbb{R})$ 的时空对称性若纳入规范群则必须破缺（产生负动能，需破缺或冻结）。$GL(5)$ 本身是物质更前面的自组织的产物。规范群必须紧致，故物理上可行的大统一规范群候选是 $SU(5) \subset GL(5,\mathbb{C})$。
