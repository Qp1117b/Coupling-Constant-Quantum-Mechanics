...
CQM 需要在离散底空间上实现 SU(5) $\to$ U(1)×SU(2)×SU(3) 的重组实现与超导候选群族 $U(1)/\mathbb Z_n$。成熟对应物：
...
| SU(5) → U(1)×SU(2)×SU(3) 重组实现 | 格点规范-Higgs 模型 | [cite:16][cite:17] | 成熟（受 Elitzur 定理与 Fradkin–Shenker 相图约束） | 需以规范固定/剩余对称语言重述；重组实现问题无现成理论 |
