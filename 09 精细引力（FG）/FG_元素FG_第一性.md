...
**核心结论**：曲率算符 $\hat{\delta}_v$ 是**Regge剖分约束**和**$[\hat{X},\hat{P}]=i\hbar$**联合作用的严格量子结果。核子量子振荡就是位置-动量对易关系的动力学表现，曲率涨落就是这些量子振荡在Regge几何上的量子投影。三模块完整重构见 §18。
...
$$\boxed{\mathcal{R}_Z = (V_Z, E_Z, F_Z, \{\bar{L}_{ij}^{(Z)}\})}$$

- $V_Z = \{1,2,3,4\}$：4个顶点（核子平衡位置）
- $E_Z$：边，经典边长 $\bar{L}_{ij}^{(Z)} = |\bar{X}_i^{(Z)} - \bar{X}_j^{(Z)}|$（依赖元素 $Z$ 的物质分布）
- $F_Z = \{\Delta\}$：三角形面，二面角由边长通过余弦定律严格确定

**每个元素有自己的剖分**：$\mathcal{R}_Z$ 依赖元素 $Z$ 的核子分布、边长结构、角亏数据。通用 $A_4$ 代数骨架（正4-单形）给出壳层结构，但元素特定的剖分数据 $\{\bar{L}_{ij}^{(Z)}, \bar{\delta}_v^{(Z)}\}$ 携带不可约化的元素信息，必须引入自组织数据才能推出关联能等元素特定物理量。
...
**严格性**：嘉当矩阵是Regge剖分约束+离散协变导数的**必然结果**。

### 3.3 简正模式与声子

> **声子是CQM的核心量子媒介**——振荡的量子、曲率涨落的源、同步的前提、时空的前提。声子的三层结构（QG/GR/FG）与统一角色详见 `01 核心理论/CQM_核心_声子理论.md`。
...
此约束将FG因果（核子空间的曲率算符）与耦合常数空间（$U(1)$紧化动量）严格锁定：$\hat{p}_u$ 的本征值由核子曲率完全决定。**Dirac约束是共形自举方程**（OPE结合律）的CQM具体化——配对顺序不影响结果（结合律）要求 $\hat{p}_u = \frac{1}{C}\sqrt{1-\beta\hat{\delta}_v}$。共形自洽锁死结构常数。
...
**结论**：周期表的"4壳层封顶"是**代数定理的物理投影**。在SU(5)破缺后的标准核子结构中，$A_4$ 是终极骨架。g壳层的稳定排布需要核子层从 $A_4$ 跃迁到 $A_5$，对应一种新的核物质相（非标准核子结构），而非普通重元素。
...
Dirac约束是**共形自举方程**（结合律）的数学必然。共形自洽锁死结构常数。
...
- 不是GL(1)+GL(4)+GL(5)直和
- 而是**单个GL(5)自守表示**
- GL(1)和GL(4)是其**子结构**（中心特征和$K$-type）
- GL(4)来自 $SO(5)\subset SU(5)$ 的旋量表示 $\mathbb{C}^4$（$\dim V = 4$），非 $SU(4)$ 破缺因子
- 分别贡献主量子数 $n$ 和轨道角动量 $l$
...
| **关联能 $E_c$ 定量计算** | He原子关联能：$\epsilon_s$ 在分子分母约去，关联能不依赖耦合常数具体值，只依赖代数系数+共形块；Cr/Cu异常：g波禁戒→S矩阵权重转移→关联能增强 $25/16\approx56\%$→能级翻转，详见§17.8 |
...
- `cqm_element_fg_strict.py`：从Regge剖分与对易关系到同步方程到本征群的严格实现
  ...
  - 关联能 $E_c$：He原子 $\epsilon_s$ 约去（普适性），Cr/Cu异常 = g波禁戒→S矩阵权重转移→增强 $25/16\approx56\%$（§17.8）
...
OPE系数由S矩阵公式严格计算（大 $k$ 极限，量子维数 $d_l = 2l+1$）：

$$|C_{l_1 l_2}^{l_3}|^2 = \frac{(2l_3+1) \, N_{l_1 l_2}^{l_3}}{(2l_1+1)(2l_2+1)}$$

| 通道 | $l_3$ | 零阶OPE系数 $C^{(0)}$ | $|C^{(0)}|^2$ | 物理意义 |
|:---:|:---:|:---:|:---:|:---|
| s波 | 0 | $1/5$ | $1/25$ | Hartree |
| p波 | 1 | $\sqrt{3}/5$ | $3/25$ | Fock |
| d波 | 2 | $1/\sqrt{5}$ | $5/25$ | 配对 |
...
**定量估计**（S矩阵严格计算）：g波通道的零阶权重为 $|C_{22}^{4,(0)}|^2 = 9/25$（§17.7.3，S矩阵公式）。此权重重新分配到4个允许通道（s,p,d,f，总权重 $16/25$），关联能修正增强：

$$\frac{|\delta E^{\text{OPE}}(\text{d}^5)|}{|\delta E^{\text{OPE, no cutoff}}|} = 1 + \frac{9/25}{16/25} = 1 + \frac{9}{16} = \frac{25}{16} \approx 1.5625$$

即 $A_4$ 截止使d⁵的关联能修正增强约 $56\%$，足以导致d⁵s¹ → d⁴s²的能级翻转。

**民主分配近似**：若假设5个通道等权重（每通道 $1/5$），增强 $= 1 + \frac{1/5}{4/5} = \frac{5}{4} = 25\%$。此近似基于 $A_4$ 对称性（4个基本权等价），给出下界。严格S矩阵计算给出 $56\%$。
...
**d波（Cr/Cu异常）结果**：关联能增强 $= 1 + \frac{9/25}{16/25} = \frac{25}{16} \approx 56\%$（S矩阵严格计算）

g波通道（$l=4$）被$A_4$截止禁戒，权重 $|C_{22}^4|^2 = 9/25$（S矩阵公式）重新分配到4个允许通道（s,p,d,f，总权重 $16/25$），增强 $25/16 \approx 56\%$，导致d⁵s¹/d¹⁰s¹能级翻转。民主分配近似给出下界 $5/4 = 25\%$。

**关键结论**：

1. **d波关联能**（Cr/Cu异常）：从CFT代数结构（fusion rules + g波禁戒 + S矩阵权重转移）**严格导出** $25/16 \approx 56\%$ 增强（民主分配近似 $5/4 = 25\%$），无经验参数，无Hamiltonian对角化
...
4. **完整同步方程与平庸退化**：上述结果（s波$E_c=0$、d波$25/16\approx56\%$增强）是**平庸方程**（通用$A_4$代数骨架）给出的。**完整同步方程** $\hat{\mathcal{S}}^{(Z)}_{\text{full}}|\Psi_Z\rangle = s_Z|\Psi_Z\rangle$ 包含元素特定的剖分 $\mathcal{R}_Z$ 和自组织数据，当忽略元素特定信息时退化为平庸方程。**完整同步方程的具体构造已完成**（§18.2）——元素特定曲率算符 + 电子反馈曲率 + 非线性自洽条件。平庸方程给出壳层结构和周期表，完整方程给出元素特定精确关联能。
...
这是**代数结构的必然推论**：Kac-Moody代数的最高权表示在半满和全满点具有最高对称性，OPE系数取极值，能量修正最大。

**定量机制**（§17.8.4）：d-d OPE的g波通道（$l_3=4$）被 $A_4$ 截止禁戒。半满/全满时，g波通道的权重（$|C_{22}^{4,(0)}|^2 = 9/25$，S矩阵公式）重新分配到允许通道（s,p,d,f），关联能修正增强 $25/16 \approx 56\%$（民主分配近似 $25\%$），足以导致能级翻转。
...
$$v_\tau^{(k,Z)} = \sqrt{1-\beta\hat{\delta}_v^{(k,Z)}} \;\longrightarrow\; p_u^{(k)} = \frac{v_\tau^{(k,Z)}}{C}, \quad C = \frac{\xi'(1)}{\xi(1)} \approx 0.0230957$$

**步骤2：同步算符 = 紧化算符在截面空间的实现**

$$\boxed{\hat{\mathcal{S}}_k^{(Z)} = \frac{L_u}{2\pi C}\sqrt{1-\beta\hat{\delta}_v^{(k,Z)}}}$$
...
**平庸方程有效性的物理依据**：代数结构结果（$Z_{\max}=118$、壳层结构）**精确正确**（由 $A_4$ Coxeter数确定，不受近似影响）；数值结果（Cr/Cu增强 $25/16 \approx 56\%$）**零阶近似下正确**；元素特定关联能**需要电子-电子同步耦合**（主要）+ 几何/归一化修正（次要）。

**关联能计算**：

$$E_c(Z) = E_h \times \rho_Z \times f_{\text{full}}(Z), \quad f_{\text{full}}(Z) = 1 + \frac{\sum_{i \in \text{occ}} (s_{Z,i} - n_i)}{\sum_{i \in \text{occ}} n_i}$$

$\rho_Z$ 从平庸方程导出（如Cr/Cu的 $25/16$），$f_{\text{full}}(Z)$ 从完整方程本征值差异导出，主要来自电子-电子同步耦合。退化时 $f_{\text{full}}(Z) \to 1$。
...
| Cr | d⁵s¹ | d-d OPE | g波禁戒→权重转移 | 增强 $25/16\approx56\%$ | §17.8.4 |
| Cu | d¹⁰s¹ | d-d OPE | 全满共振→同Cr机制 | 增强 $25/16\approx56\%$ | §17.8.4 |

**d-d OPE通道**（§17.7.3）：$2\otimes2 = 0\oplus1\oplus2\oplus3\oplus4$，g波（$l_3=4$）被 $A_4$ 截止禁戒，权重 $|C_{22}^{4,(0)}|^2=9/25$（S矩阵公式）重分配到4个允许通道（s,p,d,f），增强 $1+\frac{9/25}{16/25}=\frac{25}{16}\approx56\%$。
...
| Cr/Cu异常 | g波禁戒→权重转移 | 增强约56%（S矩阵严格）→能级翻转 | §17.12 |
...
**定义（FG纤维丛截断）**：FG纤维丛截断是从有量纲量子力学到无量纲几何的映射 $\mathcal{N}$：

$$\mathcal{N}: \left([\hat{X}_v, \hat{P}_{v'}] = i\hbar\,\delta_{vv'}\right) \mapsto \hat{\delta}_v$$

- **输入**：$[\hat{X},\hat{P}]=i\hbar$，其中 $\hat{X}$ 有长度量纲 $[L]$，$\hat{P}$ 有动量量纲 $[M][L][T]^{-1}$，$\hbar$ 有作用量量纲 $[M][L]^2[T]^{-1}$
- **输出**：$\hat{\delta}_v = \bar{\delta}_v + \hat{\delta}_v^{(1)}$，曲率算符，无量纲

#### 19.1.2 归一化的三步机制

**步骤1：简正模式对角化**（保持 $\hbar$ 量纲）

$$\hat{Q}_k = \sum_v v_k(v)\hat{X}_v, \quad \hat{\Pi}_k = \sum_v v_k(v)\hat{P}_v, \quad [\hat{Q}_k, \hat{\Pi}_{k'}] = i\hbar\,\delta_{kk'}$$

$\hat{Q}_k$ 继承长度量纲 $[L]$，$\hat{\Pi}_k$ 继承动量量纲 $[M][L][T]^{-1}$，$\hbar$ 保留作用量量纲。此步不改变量纲结构。

**步骤2：声子算符构造**（$\hbar$ 吸收到归一化常数）

$$\hat{a}_k = \sqrt{\frac{m\omega_k}{2\hbar}}\hat{Q}_k + i\sqrt{\frac{1}{2m\hbar\omega_k}}\hat{\Pi}_k, \quad [\hat{a}_k, \hat{a}_{k'}^\dagger] = \delta_{kk'}$$

$\sqrt{m\omega_k/\hbar}$ 有量纲 $[L]^{-1}$，$\sqrt{1/(m\hbar\omega_k)}$ 有量纲 $[L]$。$\hbar$ 被吸收到归一化常数中，$\hat{a}_k$ 成为无量纲算符。位置涨落继承 $\hbar$ 的残余量纲：

$$\Delta\hat{X}_v = \sum_k v_k(v)\sqrt{\frac{\hbar}{2m\omega_k}}(\hat{a}_k + \hat{a}_k^\dagger)$$

$\sqrt{\hbar/(2m\omega_k)}$ 有长度量纲 $[L]$，$(\hat{a}_k + \hat{a}_k^\dagger)$ 无量纲。

**步骤3：Regge几何非线性 + 量纲归一化**（$E_{\text{bind}}$ 吸收）

Regge几何非线性：角亏涨落正比于位置涨落平方，$(\Delta\hat{X}_v)^2$ 有量纲 $[L]^2$。通过Regge几何的二阶导数映射 $\partial^2\delta_v/\partial u_k^2 \sim m\omega_k^2/E_{\text{bind}}$，长度平方 $[L]^2$ 转换为声子能量 $\hbar\omega_k$（$[\text{能量}]$），再除以 $E_{\text{bind}}$ 归一化：

$$\boxed{\hat{\delta}_v^{(1)} = \sum_k \frac{\hbar\omega_k}{E_{\text{bind}}}|v_k(v)|^2\left(\hat{a}_k^\dagger\hat{a}_k + \frac{1}{2}\right)}$$

- $\hbar\omega_k$：声子能量（$[\text{能量}]$），来自步骤2的声子算符
- $E_{\text{bind}}$：核子结合能（$[\text{能量}]$），**量纲归一化因子**
- $\hbar\omega_k/E_{\text{bind}}$：无量纲比值
- $|v_k(v)|^2$：驻波振幅平方（无量纲，由Regge边界条件确定）
- $\hat{\delta}_v^{(1)}$：无量纲曲率算符
...
**定义**：无量纲比值 $\eta \equiv \hbar\omega_0 / E_{\text{bind}}$。
...
| $\beta = \frac{1}{4\pi}\ln(L/a)$ | 系统尺寸对数因子 | 无量纲 | 严格 |
| $N_k$ | 声子占据数（整数） | 无量纲 | 严格 |

**定理（η的代数确定性）**：$\eta$ 从 $A_4$ 代数结构 + 约束方程自洽求解严格确定，不依赖任何有量纲输入。

**证明**：右边全部是无量纲代数量（上表），$\eta$ 对所有 $k$ 取同一值（自洽条件），联立求解 $\eta$ 和 $\{N_k\}$。$\blacksquare$

**推论**：电子结构（能级、波函数、周期表）只依赖 $\eta$，不需要 $\omega_0$ 和 $E_{\text{bind}}$ 的绝对值。

### 19.3 量纲恢复的层次结构

**定义（量纲恢复）**：量纲恢复是归一化的逆映射 $\mathcal{N}^{-1}$，把无量纲代数结果转换为有量纲物理量。

**恢复公式**（§9.6.4）：

$$\boxed{E_{\text{scale}} = \frac{\hbar\omega_0}{\eta} = E_{\text{bind}} = \frac{\hbar c}{a_0 \alpha \eta}}$$

各因子来源：

| 因子 | 来源 | 量纲 | 严格性 |
|:---|:---|:---|:---|
| $\hbar$ | 预量子化线丛联络曲率 ← Regge剖分 ← $A_4$ ← GL(5) | $[M][L]^2[T]^{-1}$ | 严格 |
| $c$ | $\gamma_1\ell_{\text{QG}}$ ← GL(5) | $[L][T]^{-1}$ | 严格 |
...
$$Q = \hbar^a c^b \alpha^c \times f_{\text{CQM}}(\text{代数结构})$$

| 物理量 | 公式 | 状态 |
|:---|:---|:---|
| $\alpha$ | $\alpha^{-1} = 16384\pi/375 \approx 137.26$（§7.4） | 一阶近似（偏差0.16%） |
| $a_0$ | $\hbar/(m_e c\alpha) \sim L_u/(2\pi C\alpha)$（§17.3） | 待 $m_e$ 严格化 |

特点：需要 $\alpha$（从 $A_4$ 导出），但计算无量纲比例时不需要 $m_e$ 绝对值。

#### 19.3.3 层次3：原子能级标度（需要 $\alpha$ 和 $m_e$）

$$\boxed{E_n = \alpha^2 m_e c^2 \times \epsilon_n = E_h \times \epsilon_n}$$

其中 $E_h = \alpha^2 m_e c^2 \approx 27.2$ eV 是Hartree能量尺度，$\epsilon_n = -1/(2n^2)$ 是无量纲能级（从CQM代数导出）。

特点：需要 $\alpha$（已有，一阶近似）和 $m_e$（待严格化，§19.4给出涌现机制）。

#### 19.3.4 层次4：关联能绝对值（需要完整同步方程 + $\alpha$ + $m_e$）

$$\boxed{E_c(Z) = E_h \times \rho_Z \times f_{\text{full}}(Z)}$$

其中：
- $E_h = \alpha^2 m_e c^2$：Hartree能量尺度（层次3标度恢复）
- $\rho_Z$：增强因子（从平庸同步方程导出，无量纲，如Cr/Cu的 $25/16$）
- $f_{\text{full}}(Z)$：元素特定因子（从完整同步方程导出，无量纲）
...
1. **物理定义**：$a_0 = \hbar/(m_e c\alpha)$
2. **CQM导出**（§17.3）：$a_0 \sim L_u/(2\pi C\alpha)$

CQM导出式中 $L_u, C, \alpha$ 均无量纲，因此 $a_0$ 的长度量纲必须来自一个基本长度标度 $\ell_0$：
...
$$\boxed{E_n = E_{\text{scale}} \times \epsilon_n = \frac{\hbar\omega_0}{\eta} \times \epsilon_n}$$
...
**计算步骤**：
1. 从平庸同步方程计算增强因子 $\rho_Z$（已完成，如Cr/Cu的 $25/16$）
...
用**通用** $E_{\text{bind}}$ 处理所有元素：

$$\hat{\delta}_v^{(1)} = \sum_k \frac{\hbar\omega_k}{E_{\text{bind}}}|v_k(v)|^2(\hat{a}_k^\dagger\hat{a}_k + 1/2)$$

$E_{\text{bind}}$ 不依赖元素 $Z$。归一化抹除了元素特定的能量标度信息 → 平庸同步方程 → 壳层结构 + 周期表 + 增强因子。

#### 19.6.2 完整归一化（目标）

每个元素 $Z$ 有自己的 $E_{\text{bind}}^{(Z)}$ 和 $\omega_k^{(Z)}$：

$$\hat{\delta}_v^{(1,Z)} = \sum_k \frac{\hbar\omega_k^{(Z)}}{E_{\text{bind}}^{(Z)}}|v_k^{(Z)}(v)|^2(\hat{a}_k^\dagger\hat{a}_k + 1/2)$$

$E_{\text{bind}}^{(Z)}$ 和 $\omega_k^{(Z)}$ 依赖元素 $Z$ 的特定剖分 $\mathcal{R}_Z$ 和自组织数据。归一化保留元素特定的能量标度信息 → 完整同步方程 → 元素特定精确关联能。

#### 19.6.3 退化关系

$$\hat{\delta}_v^{(1,Z)} \xrightarrow{\text{忽略元素特定信息}} \hat{\delta}_v^{(1)}$$

$$\hat{\mathcal{S}}^{(Z)}_{\text{full}} \xrightarrow{\text{忽略元素特定信息}} \hat{\mathcal{S}}_{\text{trivial}}$$
...
```
有量纲量子力学                    FG纤维丛截断                    无量纲代数
──────────────    ─────────────────────────────    ──────────────
[X̂, P̂] = iℏ      步骤1: 简正模式 (保持ℏ)           δ̂ᵥ (无量纲曲率)
 X̂: [长度]        步骤2: 声子算符 (吸收ℏ)            │
 P̂: [动量]        步骤3: Regge非线性 + ÷E_bind       │
 ℏ: [作用量]              ↓                          ↓
                    η = ℏω₀/E_bind              同步方程
                    (无量纲比值)                ŜΨ = nΨ (无量纲)
                    从A₄代数严格确定                  │
