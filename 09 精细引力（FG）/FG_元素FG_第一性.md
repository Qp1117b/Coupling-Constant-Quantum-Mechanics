...
同步方程为每个本征群 $G_k$ 输出一个耦合常数：

$$\boxed{g_k = g_0\exp\left(-\frac{n_k}{n_{\text{ref}}}\right), \quad n_k = \frac{L_u}{2\pi C}\sqrt{1-\beta\delta_v^{(k)}}}$$

$g_k$ 是同步方程的**输出**，不是输入参数。每个本征群有自己的耦合常数，编码该群的全部谱信息。

### 9.2 耦合常数 → Casimir本征值 → 角动量

$g_k$ 编码本征群 $G_k$ 的 Casimir 本征值 $C_k$：

$$\boxed{C_k = -n_{\text{ref}}\ln\frac{g_k}{g_0} = n_k}$$
...
$l_k$ 是耦合常数 $g_k$ 的输出——通过 Casimir 本征值严格确定。
...
**填充顺序**由同步成本 $s_k = n_k + l_k$（Madelung规则）排序。

**总电荷数**是同步方程输出的耦合常数的函数：

$$\boxed{Z = \sum_{k=1}^{4} N_k^{\max}(g_k) = \sum_{k=1}^{4} 2\sqrt{4n_k - 2}}$$
...
总曲率 δ̂_v = δ̄_v + δ̂_v⁽¹⁾
        ...
        │
        ├── 耦合常数 g_k → Casimir C_k → 角动量 l_k
        │
        ├── 电子容量 N_k^max = 2(2l_k+1) = 2√(4n_k-2)
        │
        └── 费米填充 → Z = Σ N_k
```
...
| 耦合常数→Casimir→$l_k$→$N_k^{\max}$ | **结构严格**（$g_k \to C_k \to l_k \to 2(2l_k+1)$），$n_k = C_k$ 正则化条件待推导 |
...
| **耦级→Casimir正则化** | 🔴 致命 | $n_k = C_k = l(l+1)+3/4$ 的正则化条件待从同步方程严格推导 |
...
- `cqm_element_fg_strict.py`：从Regge剖分与对易关系到同步方程到本征群的严格实现
  ...
  - 耦合常数 $g_k$ → Casimir $C_k = n_k$ → 角动量 $l_k$
  - 电子容量 $N_k^{\max} = 2(2l_k+1) = 2\sqrt{4n_k-2}$
  - 费米填充 → $Z = \sum N_k$
