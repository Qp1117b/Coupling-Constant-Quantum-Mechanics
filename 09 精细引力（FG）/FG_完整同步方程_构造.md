...
**退化**：Born-Oppenheimer近似（$m_p \gg m_e$，核子不感受电子反馈）下 $\Delta\hat{\delta}_v^{\text{e-n}} \approx 0$。注意这不是 $\rho_Z \to 0$（无电子），而是质子质量主导使电子反馈可忽略。
...
1. **Born-Oppenheimer近似**：$\Delta\hat{\delta}_v^{\text{e-n}} \approx 0$（核子不感受电子反馈）
   - 物理依据：$m_p/m_e \approx 1836 \gg 1$，核子运动比电子慢得多
   - 电子反馈是 $O(m_e/m_p) \sim 10^{-3}$ 修正
...
2. **数值结果**（Cr/Cu增强25%）：**零阶近似下正确**——三重近似是好的近似：
   - Born-Oppenheimer：$m_e/m_p \sim 10^{-3}$，电子反馈修正 $\sim 0.1\%$
...
**物理上**：平庸方程是完整方程在**质子质量主导**（$m_p \gg m_e$）+ **核子分布接近对称**（$A_4$ 近似）+ **结合能变化缓慢**三个条件下的零阶近似。
