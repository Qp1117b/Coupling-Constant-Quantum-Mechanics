...
| 缺口 | 描述 | 状态 |
|:---|:---|:---|
| 定理 1.1 | $☯$ 的零点求和表示（谱表示，条件于 RH）$☯ = \sum 1/(\gamma_n^2+1/4)$，需 ξ 的 Hadamard 乘积；定义 $☯ = \xi'(1)/\xi(1)$ 不依赖 RH | 依赖 `SpectralGeometry.RiemannXi`（当前编译失败），待该模块修复 |
...
| §6 构造 10.1 | $\ln(\prod_p \det(D_p)) = 1/☯$ 的启发式对应 | 启发式；库内仅给出其条件推论 `𝒥 = exp(−2/☯)` |
