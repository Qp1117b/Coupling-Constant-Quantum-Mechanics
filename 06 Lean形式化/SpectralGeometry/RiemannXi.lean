...
> **发生学分层定位（2026-08）**：本文件形式化的 ζ/ξ 谱结构属
> **电磁因子（GL(1)）层**——黎曼猜想是广义黎曼猜想在 GL(1)
> （SU(5) 破缺后 U(1) 电磁因子）的特例。质数分布的基态同步是
> SU(5)（来自 GL(5) 自守表示的紧化）；在 GL(n) 层级谱中，物质
> 自组织选中 GL(5) 正是物质自组织的体现。本文件不涉及基态层
> 的 GL(5) 自守 L 函数（其形式化待构造）。
