...
- `superconductingOrderTensor_pos`：张量超导序参量（§6.8 公式在 A₄ 本征谱上的 Tr_𝒞）严格为正
...
- `superCartan_symmetric` / `identityBlock_symmetric` / `cartanA4Stack_symmetric`：**组装对称性（SPAF §3.5）**——对称矩阵叠加保对称、标量倍单位矩阵块对称、A₄ 直接拼接保实对称（分子超嘉当矩阵 C_mol 幺正约束的严格版）
- `neutronCartan_symmetric` / `neutronCartan_zero_eq_proton` / `neutronCartan_diag00` / `neutronCartan_diag_ne00` / `neutronCartan_diag00_pos`：**中子缺陷嘉当矩阵 C_n = A₄ − ε·diag(1,0,0,0)（SPAF §3.2）**——C_n 实对称、ε=0 退化为质子（cartanHamiltonian）、缺陷位对角元 = 2−ε、非缺陷位保持 2、ε<2 时缺陷位对角元为正（谱判据的必要条件）
...
- `flavorFlowRate_nonneg`：**味空间概率流（SPAF §5 步骤 13）**——Γ_rel = Σ_{a,b} |dP_{a→b}/dτ| ≥ 0（锁定条件 Γ_rel(T_lock) → 0 与超导判据 T_c = T_lock 的非负性前提）
- `reggeEdgeLength_pos`：**Regge 边长（SPAF §5 步骤 15）**——l_e = κ/√λ_e > 0（正边长 = Regge 微积分良定义的必要条件）
...
- G14：SPAF §4.2 宏观频谱判据的 Rayleigh/Weyl 型谱引理未建立（数学库未提供）——
  ...
  （`neutronCartan_not_posDef_of_five_fourths_le`）。**唯一残留缺口**：区间
  [1, 5/4) 内正定保持（等于 C_n 行列式 det = 5 − 4ε > 0 的余子式展开/正定判据 mod
  Sylvester，涉及 4 阶矩阵全子式族，未在数学库中直接建立；及 γ_min = (3−√5)/2 ≈ 0.699
  作为严格阈值 min{2, λ₂, ...} 的进一步认证）
- G15：SPAF §3.4 因果耦合指数形式/截断长度 λ 的 L0 微观来源未证（以公理输入处理）
- G16：SPAF §5 动力学协调形式（Γ_rel(T_lock) → 0 的锁定方程）与 Regge 变分原理未形式化
