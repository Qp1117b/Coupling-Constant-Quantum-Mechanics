...
> **Wyler 常数嵌入 CQM 的 SU(5) 重组实现框架，对精细结构常数 $\alpha$ 的来源给出更精确的群论表达。**
...
5. **GL(5) 整体反映**：$\alpha$ 是 GL(5) 整体结构的反映
...
| $\Vert W(A_4)\Vert$（Weyl 群阶） | 120 | $S_5$ 对称性，CQM 已用 $W_m = 5\cdot 2^{m-1}$ |
...
| | CQM 粗略表达式 | Wyler 常数 |
|:---|:---|:---|
| **使用的信息** | 本征值**近似**比例 $9:4:1$ | Coxeter 数 $h$、秩 $r$、Weyl 群阶 $\Vert W\Vert$、迹 $\text{tr}(C)$ |
...
CQM 已经断言"$\alpha$ 是 GL(5) 整体的反映"。Wyler 常数为这个断言提供了**更强的证据**：
