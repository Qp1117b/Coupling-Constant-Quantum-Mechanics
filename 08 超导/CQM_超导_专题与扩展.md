...
**统一公式**：饱和电子数 $= 2(2l+1)$，$l=0,1,2,3$，来自 $SO(3) \times SU(2)$（轨道×自旋）。$A_4$ 是 $SO(3)$ 的有限子群，$A_4 \to SO(3)$ 涌现给出连续对称性。p和d的6和10从 $SU(4)$ 表示论得到（$SU(5) \supset SU(4)$），s的2来自 $SU(2)$ 自旋（$SU(5)$ 破缺产物），f的14来自 $G_2$ 伴随表示——s和f的群论来源需要进一步从核子结构导出。**在元素FG同步方程框架中**（`09 精细引力（FG）/FG_元素FG_第一性.md` §9），每个本征群的耦合常数 $g_k$ 通过 Casimir 本征值 $C_k = n_k$ 决定角动量 $l_k$，从而决定饱和电子数 $N_k^{\max} = 2(2l_k+1) = 2\sqrt{4n_k-2}$——耦合常数是壳层容量的来源。
