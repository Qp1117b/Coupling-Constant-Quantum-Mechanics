...
| L函数 | 零点差 | 物理含义 |
|:------|:-------|:---------|
| GL(1): $\zeta(s)$ | $\gamma_2 - \gamma_1 = 6.887315$ | 黎曼零点差（电磁配对标度，进入 $T_c$ 公式） |
...
| 能动张量·GL(2) | $C_{\text{ANISO}} \cdot \text{aniso}$ | GL(2) | 嘉当矩阵各向异性（$j(j+1)$ Casimir） |
...
**发生学分层**（§9）：SU(5) 重组实现后 $\to U(1)_{\text{em}} \times SU(2)_{\text{spin}} \times SU(3)_{\text{color}}$，对应 GL(1)/GL(2)/GL(3) 三个自守对偶残留因子。超导只涉及 GL(1) 和 GL(2)：
...
2. **配对子流形维度 $d_{\text{pair}}$ 的 CQM 推导**：SU(5) → 点群重组实现，$U(1)^4$ 四个因子中非平庸方向的数目决定配对维度。代理量 $d_{\text{pair}} = 3 - c \cdot \ln(G \cdot N)$（$G$ 大 → 质量差异大 → 各向异性 → 降维）。铜氧化物/铁基 $d≈2$，有机 $d≈1.5$，常规超导 $d≈3$。

3. **配对子流形量子曲率**：$\kappa_{\text{pair}} = \theta_D \cdot \sqrt{M/(B \cdot l)}$（Debye 频率标度 × 弹性响应）。

4. **SU(2)/SU(3) 混合角**：$\sigma_{\text{eff}} = \tanh(\ln G / 5)$（SU(5) 重组实现非平庸性的代理）。
...
**未闭合（当前精度上限的原因）**：
- **$\gamma_{\text{nearest}}$ 的独立确定**（解决）：$\gamma_{\text{nearest}}$ 已从13维CQM自然量连续推导（嘉当矩阵谱间隙、角亏涨落等），纯第一性LOOCV中位76.6%，d波中位14%——这是**CQM纯第一性框架的当前精度**。
...
**非常规 d/p 波超导**：自旋结构非平凡，通过 $j(j+1)$ Casimir 修正（$\hat{\mathcal{S}}_2$ 的独立离散谱 $\eta_m$ 的代数参数）进入 $T_c$ 计算。$\gamma_n$ 映射从嘉当矩阵谱间隙第一性推导。当前 CQM 用 $j(j+1)$ Casimir 修正给出非常规超导中位 33.3% 精度。

**完整超导公式形式**（CQM 完全体）：

$$T_c = F\left(\gamma_2-\gamma_1;\ j(j+1);\ \delta_v,\ \theta_D\right)$$

**与零点猜想的关系**：$C_1$ 需黎曼猜想（GL(1)）。**超导完整理论需要 RH 成立。**
...
**机制链。** 从 QG 基态紧化结构（GL(5)/SU(5)）重组实现后电磁因子（GL(1)）子结构到超导相变的具体机制猜测：
...
- **QG再现（非创造）**：同步算符 = QG 基态紧化结构（GL(5)/SU(5)）重组实现后电磁因子（GL(1)）子结构的再现，超导相变 = 电磁因子黎曼共振谱在凝聚态层级的显现。不是超导层级新创造的机制，是先在结构的显现。**FG 本来就是激活 QG**——角亏涨落打开的前几何自由度，在超导再锁定时以同步算符形式重新组织。
...
超导是**耦合空间**的同步（$\gamma_n$ 直接给出耦级），氢原子是**实空间**的同步（$n = N(\gamma_n)$ 给出空间尺度 $n^2 a_0$，$1/n^2$ 给出能级）。同步算符是**普遍的**（本征值始终是 $\gamma_n$），导出方式取决于物理系统。两者都是 QG 基态紧化结构（GL(5)/SU(5)）重组实现后电磁因子（GL(1)）子结构在不同层级的显现。
...
- **QG再现（非创造）**：氢原子能级 = 同步算符在原子层级的显现 = QG 基态紧化结构（GL(5)/SU(5)）重组实现后电磁因子（GL(1)）子结构的再现。超导能级 = 同步算符在超导层级的显现 = 同一子结构的再现。两者都是先在潜能的显现，不是各自创造新结构。
