...
> 说明：温度为外部因素，改变晶胞振荡声子数、经自组织-再生产环节进入超导；压强为外部几何约束；磁场通过 (U(1)) 阿贝尔正则动量锁定探测。机制本体见主文档 §11、§12。
...
> 说明：主文档 §11.10 保留同步算符构造、显式映射、$T_c$ 闭式推导、高阶跃迁与自由能 $T_c$ 推导链等核心内容；以下为 $K_0$、质量依赖、Weyl 根向量、希格斯 $s=1/2$、高阶谱矩、分化树整合、GL(1)/GL(2) 分层与双重谱等深度第一性推导细节。
...
> ※ 本节（11.10 ★★ 联合优化框架（当前最佳Tc预测））已移至 [CQM_超导_专题与扩展.md](CQM_超导_专题与扩展.md)，正文详见该文档。
> ※ 本节（11.10 ★ $\gamma_{\text{cat}}$的第一性确定（消除类别依赖））已移至 [CQM_超导_专题与扩展.md](CQM_超导_专题与扩展.md)，正文详见该文档。
...
> ※ 本节（11.10 ★★★★ 条件数各向异性修正：从能动张量各向异性导出配对方向选择性）已移至 [CQM_超导_专题与扩展.md](CQM_超导_专题与扩展.md)，正文详见该文档。
> ※ 本节（11.10 ★★★★ f电子抑制修正：从局域化电子分数导出正确抑制强度）已移至 [CQM_超导_专题与扩展.md](CQM_超导_专题与扩展.md)，正文详见该文档。
> ※ 本节（11.10 ★★★★ 当前框架精度与根本限制）已移至 [CQM_超导_专题与扩展.md](CQM_超导_专题与扩展.md)，正文详见该文档。
...
底空间几何已指定对应椭圆曲线（A4 Weyl 群 $S_5$ 子群分类：$A_4$→p 波→$y^2=x^3-1$，$D_4$→d 波→$y^2=x^3-x$）。$C_f = 0$ 已证明。零点差已计算。完整推导见 `08 超导/CQM_超导_FG层级同步算符体系.md`。

> ※ 本节（11.11 全面超导体测试：普适超导判据）已移至 [CQM_超导_专题与扩展.md](CQM_超导_专题与扩展.md)，正文详见该文档。
