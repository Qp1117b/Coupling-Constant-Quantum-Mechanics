...
- **数值验证结论**：GL(2) 零点差与 $j(j+1)$ Casimir 在当前线性回归框架中携带相近信息量（LOOCV 中位 33.3% vs 33.1%，工作包1 重新标定），由此推断两者同源是**解释性工作假设**（非严格证明的数学同一性）。当前框架中 d 波/p 波区分主要来自 $\gamma_n$（黎曼零点序号映射），而非 GL(2) 修正项——$\gamma_n$ 映射已从三条链的输出第一性推导（已闭合：从材料几何→嘉当矩阵→Weyl群→GL(2)零点差→γ_n，纯第一性）。验证脚本：`cqm_analysis/cqm_first_principles_strict.py`。
...
其中前因子 $C_{\text{GAMMA}} = e^{1/\beta} \cdot \alpha_{\text{fs}}^3 \cdot \hbar^{-1/4} k_B^{1/8} m_e^{-1/4} a_0^{-1/2} \approx 7.78 \times 10^{11}$ 已从CQM第一性推导（§开放问题已闭合），$\alpha_{\text{fs}}$=精细结构常数，$\beta=8\pi+1$=主丛曲率参数。
...
**当前状态**：重费米子修正理论推导已完成，但在框架**对称误差标准**（$e = \max(r, 1/r) - 1$，要求 $0.5T_c < T_{\text{pred}} < 2T_c$）下，重费米子修正过度抑制铁基超导体（如SmFeAsO被抑制60倍），导致总体精度从52.8%降至49.2%。在**绝对误差标准**（$e = |T_{\text{pred}} - T_c|/T_c$，不惩罚低估）下，修正有效（+4个材料）。**未整合到主框架**，待解决低估惩罚问题。
...
**已闭合**：底空间几何已指定对应椭圆曲线（A4 Weyl 群 $S_5$ 子群分类：$A_4$→p 波→$y^2=x^3-1$，$D_4$→d 波→$y^2=x^3-x$）。$C_f = 0$ 已证明。零点差已计算。完整推导见 `08 超导/CQM_超导_底空间推导与形式化.md`。
...
| ✅ 已解决 | **元素周期表推导**（§11.7）：CQM本体论内在要求能推周期表和电子壳层分布（否则本体论不一致）。**框架内构造性推导**：$SU(5)$发生学→电子/核子形成；$A_4$型($SU(5)$, $h=5$)→Coxeter数限制$l=0,1,2,3$；$SU(5)\supset SU(4)$→$4\otimes4=10\oplus6$→p=6,d=10；$SU(2)$自旋=$SU(5)$破缺产物→$\times2$；壳层饱和数$2,6,10,14$（不需要$G_2$）；周期长度$2,8,18,32$；Madelung规则$E(n,l)\sim N(\gamma_n)+l=n+l$（黎曼式同步算符）；电子FG=前中子-前质子底空间；壳层鲁棒性定理（§11.8）；$\delta(Z,N)=1-\varepsilon_0 N/(Z+N)$（§11.8）；$V_{\text{element}}=V_0+L_{\text{orbital}}$显式构造（$\varphi_l=(l/\lambda_l)\Pi_l$，$Y_l^m$从$SU(4)\to SO(3)$涌现），填充顺序与周期表一致；洪特规则三条定量解释（$E_{\text{sync}}=\text{const}-\frac{\lambda_{\text{spin}}}{2}S(S+1)-\frac{\lambda_{\text{orb}}}{2}L(L+1)$，$p^1$–$p^6$、$d^1$–$d^{10}$验证一致） | 框架内构造性推导给出，严格证明待独立复现 |
