...
# 理论C_G
KB_PRECISE = 1.380649e-23; HBAR_PRECISE = 1.054571817e-34; AMU_PRECISE = 1.66053906660e-27; A0 = 5.29177210903e-11
DIM_PART = KB_PRECISE**(1/8) * HBAR_PRECISE**(-1/4) * AMU_PRECISE**(-1/4) * A0**(-1/2)
C_G_EMPIRICAL = 7.77e11
C_G_THEORY = DIM_PART * math.exp(-4 * math.pi)

print(f"C_G经验 = {C_G_EMPIRICAL:.4e}")
print(f"C_G理论 = {C_G_THEORY:.4e} (量纲×exp(-4π))")
print(f"比值 = {C_G_EMPIRICAL/C_G_THEORY:.4f}")
...
def solve_tc(formula, c_gamma):
    ...
    f_supp = BETA / math.sqrt(3)  # 理论值
    d0_supp = 3  # l_d + 1
    suppress = math.exp(-f_supp * af['f']) * math.exp(-d0_supp * af['d0'])
...
for label, cg in [("经验C_G=7.77e11", C_G_EMPIRICAL), ("理论C_G=量纲×exp(-4π)", C_G_THEORY)]:
    errs = []; cat_errs = {}
    for d in data:
        tc = solve_tc(d['f'], cg)
        if tc is None or tc <= 0: continue
        e = sym_err(tc, d['tc'])
        errs.append(e)
    ...
    errs.sort()
    print(f"\n{label}:")
    print(f"  中位{errs[len(errs)//2]*100:.1f}%, 2倍内{sum(1 for e in errs if e<=1.0)/len(errs)*100:.1f}%, 5倍内{sum(1 for e in errs if e<=4.0)/len(errs)*100:.1f}%")
    for cat, ce in sorted(cat_errs.items(), key=lambda x: -len(x[1])):
        if len(ce) < 5: continue
        ce.sort()
        w2 = sum(1 for e in ce if e <= 1.0) / len(ce) * 100
        print(f"    {cat:20s} n={len(ce):3d}: 2倍内{w2:.0f}%")