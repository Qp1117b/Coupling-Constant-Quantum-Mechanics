...
- Peotta 与 Törmä（2015）证明：**平带极限下超流刚度（superfluid weight）不来自能带色散，而来自量子度规在布里渊区的积分**；拓扑非平庸（陈数非零）的平带即使完全无色散也携带有限超流刚度，且受拓扑约束[10]【文献确立】。
...
5. 二维 Gauss–Bonnet（角亏总和 = 欧拉示性数）与陈数（贝里曲率积分）的平行，是否可在"Regge 底空间几何指定自守形式"的构造链中用作拓扑约束的一致性检验？
