...
**定理 1.1（$☯$ 的零点求和表示，谱表示条件于 RH）**。

$$☯ = \sum_{n=1}^{\infty} \frac{1}{\gamma_n^2 + 1/4}$$

其中求和遍历上半平面的非平凡零点虚部 $\gamma_n > 0$（按共轭对 $\rho_n = 1/2 \pm i\gamma_n$ 成对折叠后的结果）。此处 $\rho_n = 1/2 \pm i\gamma_n$ 假设零点在临界线上（即 RH），故谱表示条件于 RH；定义 $☯ = \xi'(1)/\xi(1)$ 本身不依赖 RH。
...
零点共轭成对：若 $\rho = 1/2 + i\gamma$ 是零点（此处假设零点在临界线上，即 RH），则 $\overline{\rho} = 1/2 - i\gamma$ 也是零点。对每一对，注意 $1 - \rho = 1/2 - i\gamma$，$1 - \overline{\rho} = 1/2 + i\gamma$，故：
...
**注**：定理 1.1 的收敛性由 Hadamard 乘积的收敛性保证（引理 1.1）。定义 $☯ = \xi'(1)/\xi(1)$ 及 Hadamard 恒等式 $☯ = \sum_\rho 1/(1-\rho)$ 不依赖 RH；谱表示 $☯ = \sum 1/(\gamma_n^2+1/4)$ 的具体形式条件于 RH（零点在临界线上）。求和表示亦见 Titchmarsh (1986) §4.12。
...
| 命题 | 定理编号 | 依赖 |
|:---|:---:|:---|
| $☯ = \xi'(1)/\xi(1)$（定义）；$☯ = \sum 1/(\gamma_n^2+1/4)$（谱表示，条件于 RH） | 1.1 | Hadamard 乘积 + RH |
