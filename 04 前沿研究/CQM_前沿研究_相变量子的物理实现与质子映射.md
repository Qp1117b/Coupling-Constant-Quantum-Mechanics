...
**性质**：前沿研究记录（外部文献数学事实核查 + 模型核查 + CQM 体系映射）

**诚实声明**：本文档分三层内容，其成立性不同。（1）**数学事实层**：$\lambda_1 = ☯$、$B = -☯$ 两个等式均为已验证的数学事实（30 位精度确认），不依赖 CQM 框架任何假设。（2）**模型核查层**：§3 的排除清单基于对候选模型全文的实际核查，核查结论是确定性的"该文献是否出现该常数"。（3）**结构类比层**：§6 的质子映射是结构类比，不是推导；映射不提供 $m_p$ 数值，不改变 $G_N$ 推导路径的依赖方向（§6.6），且不进入 QG 框架（QG 框架不使用黎曼零点）。三层界限在正文中逐节标注。
...
**本节性质：结构类比层**。以下全部内容为结构类比与分析，不是推导。类比的用途是为相变量子 ☯︎ 的物理实现提供候选图像与研究方向，不为任何数值结果提供依据。
...
问题由此产生：$G_N$ 公式中出现的 $☯^2$ 与 $\exp(-2/☯)$ 来自谱面孔，而质子是质量层对象——**谱面孔的普适权重如何落实到质量层的复合单元上**？§6.2–§6.5 给出候选图像。
...
- **不提供 $m_p$ 数值**：LeClair 模型不含任何能产出 $m_p$ 的结构。$m_e/m_p$ 的推导链在声子理论 §8.3（中子/质子振荡），与本映射是两条不同任务，不可混同。
...
本映射在 CQM 体系中的定位，按 L 函数三层角色（数学事实层 / 结构使用层 / 推导路径层）划分：

| 层 | 内容 | 状态 |
|:---|:---|:---|
| 数学事实层 | $\lambda_1 = ☯$、$B = -☯$（§2） | 已验证（30 位精度） |
| 结构使用层 | LeClair 模型作为 ☯︎ 谱流物理实现的候选映射（§5–§6） | 本文档主体，结构类比 |
| 推导路径层 | **不构成 $G_N$ 推导的一部分** | 见下 |

**依赖方向不改变**：`CQM_前沿研究_质量研究思路汇总` §7 明确 $G_N$ 公式路径与自守路径互斥、不得互为前提。本映射不引入新的推导路径——它不声称"由 LeClair 模型推出 $G_N$"，也不声称"由质子推出 ☯︎"。☯︎ 的提取来源仍是 $\mathrm{GL}(n)$ 平凡自守形式 $L$ 函数（质量研究思路汇总 §6.3），LeClair 模型只是这一提取结果在外部物理文献中的**实现候选**。
